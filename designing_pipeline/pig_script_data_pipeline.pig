...
logs = LOAD '/user/dir4/hive_bundle/Omniture.0.tsv' USING PigStorage('\t') 
AS 
...
store logs2 into '/user/dir4/hive_bundle/pig_out_parsed_logs';


---load products
products = LOAD '/user/dir4/hive_bundle/products.tsv' 
USING PigStorage('\t') AS (url:chararray,category:chararray);

store products into '/user/dir4/hive_bundle/pig_out_products';


--load users

users = LOAD '/user/dir4/hive_bundle/users.tsv' 
USING PigStorage('\t') AS (SWID:chararray,BIRTH_DT:chararray,GENDER_CD:chararray);
store users into '/user/dir4/hive_bundle/pig_out_users';
